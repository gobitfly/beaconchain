...
func InitBigtable(project, instance, chainId, redisAddress string) (*Bigtable, error) {
	if utils.Config.Bigtable.Emulator {
		...
		log.LogInfo("using emulated local bigtable environment, setting BIGTABLE_EMULATOR_HOST env variable to %s:%d", utils.Config.Bigtable.EmulatorHost, utils.Config.Bigtable.EmulatorPort)
		...
		if err != nil {
			log.LogFatal(err, "unable to set bigtable emulator environment variable", 0)
		}
	}
	...
	for {
		...
		case mut, ok := <-bigtable.machineMetricsQueuedWritesChan:
			...
			if len(muts.Keys) >= batchSize || !ok && len(muts.Keys) > 0 { // commit when batch size is reached or on channel close
				log.LogInfo("committing %v queued machine metric inserts (trigger=batchSize, ok=%v)", len(muts.Keys), ok)
				err := bigtable.WriteBulk(muts, bigtable.tableMachineMetrics, batchSize)

				if err == nil {
					muts = types.NewBulkMutations(batchSize)
				} else {
					log.LogError(err, "error writing queued machine metrics to bigtable", 0)
				}
			}

			if !ok { // insert chan is closed, stop the timer and exit
				tmr.Stop()
				log.LogInfo("stopping batched machine metrics insert")
				return
			}

		case <-tmr.C:
			if len(muts.Keys) > 0 {
				log.LogInfo("committing %v queued machine metric inserts (trigger=timeout)", len(muts.Keys))
...
func getMachineMetrics[T types.MachineMetricSystem | types.MachineMetricNode | types.MachineMetricValidator](bigtable Bigtable, process string, userID uint64, limit, offset int, marshler func(data []byte, machine string) *T) ([]*T, error) {
	...
	gapSize := utils.GetMachineStatsGap(uint64(limit))
	err := bigtable.tableMachineMetrics.ReadRows(ctx, gcp_bigtable.PrefixRange(rangePrefix), func(r gcp_bigtable.Row) bool {
		success, _, machine, _ := machineMetricRowParts(r.Key())
		if !success {
			return false
...
func machineMetricRowParts(r string) (bool, uint64, string, string) {
	...
	if err != nil {
		log.LogError(err, "error parsing slot from row key", 0, map[string]interface{}{"row": r})
		return false, 0, "", ""
	}
...
func (bigtable *Bigtable) SaveProposalAssignments(epoch uint64, assignments map[uint64]uint64) error {
	...
	log.LogInfo("exported proposal assignments to bigtable in %v", time.Since(start))
	return nil
}
...
func (bigtable *Bigtable) SaveAttestationDuties(duties map[types.Slot]map[types.ValidatorIndex][]types.Slot) error {
	...
	if bigtable.LastAttestationCache == nil {
		...
		log.LogInfo("initialized in memory last attestation slot cache with %v validators in %v", len(bigtable.LastAttestationCache), time.Since(t))
	}
	bigtable.LastAttestationCacheMux.Unlock()
			...
			for _, inclusionSlot := range inclusions {
				...
				if inclusionSlot != MAX_CL_BLOCK_NUMBER && uint64(attestedSlot) > bigtable.LastAttestationCache[uint64(validator)] {
					...
					if mutLastAttestationSlotCount == MAX_BATCH_MUTATIONS {
						...
						mutLastAttestationSlot = gcp_bigtable.NewMutation()
						mutLastAttestationSlotCount = 0
						log.LogInfo("applyied last attestation slot mutations in %v", time.Since(mutStart))
					}
				}
			}
			bigtable.LastAttestationCacheMux.Unlock()
	...
	log.LogInfo("exported %v attestations to bigtable in %v", mutsInclusionSlot.Len(), time.Since(start))
	return nil
}
...
func (bigtable *Bigtable) SaveProposal(block *types.Block) error {
	...
	log.LogInfo("exported proposal to bigtable in %v", time.Since(start))
	return nil
}
...
func (bigtable *Bigtable) SaveSyncComitteeDuties(duties map[types.Slot]map[types.ValidatorIndex]bool) error {
	...
	if len(duties) == 0 {
		log.LogInfo("no sync duties to export")
		return nil
	}
	...
	log.LogInfo("exported %v sync committee duties to bigtable in %v", muts.Len(), time.Since(start))
	return nil
}

// GetMaxValidatorindexForEpoch returns the highest validatorindex with a balance at that epoch
	...
	for i := 0; i < len(validators); i += batchSize {
		...
		g.Go(func() error {
			...
			handleRow := func(r gcp_bigtable.Row) bool {
				...
				if err != nil {
					log.LogError(err, "error parsing epoch from row key", 0, map[string]interface{}{"row": r.Key()})
					return false
				}

				validator, err := bigtable.validatorKeyToIndex(keySplit[1])
				if err != nil {
					log.LogError(err, "error parsing validator index from row key", 0, map[string]interface{}{"row": r.Key()})
					return false
				}
				resMux.Lock()
			...
			// log.LogInfo("retrieved data for validators %v - %v", vals[0], vals[len(vals)-1])
			return nil
		})
	}
...
func (bigtable *Bigtable) getValidatorBalanceHistoryV1(validators []uint64, startEpoch uint64, endEpoch uint64) (map[uint64][]*types.ValidatorBalance, error) {
	...
	handleRow := func(r gcp_bigtable.Row) bool {
		...
		for _, ri := range r[VALIDATOR_BALANCES_FAMILY] {
			...
			if err != nil {
				log.LogError(err, "error parsing validator from column key", 0, map[string]interface{}{"row": ri.Column})
				return false
			}
	...
	for i := 0; i < len(validators); i += batchSize {
		...
		g.Go(func() error {
			...
			err := bigtable.tableValidatorsHistory.ReadRows(ctx, ranges, func(r gcp_bigtable.Row) bool {
				...
				validator, err := bigtable.validatorKeyToIndex(keySplit[1])
				if err != nil {
					log.LogError(err, "error parsing validator from row key", 0, map[string]interface{}{"row": r.Key})
					return false
				}
...
func (bigtable *Bigtable) GetValidatorMissedAttestationsCount(validators []uint64, firstEpoch uint64, lastEpoch uint64) (map[uint64]*types.ValidatorMissedAttestationsStatistic, error) {
	...
	// log.LogInfo("retrieved missed attestation history for epochs %v - %v", firstEpoch, lastEpoch)
	...
	for validator, history := range data {
		for _, attestation := range history {
			...
			if attestation.InclusionSlot > 0 {
				// log.LogInfo("adding %v for epoch %v %.2f%%", attestation.InclusionSlot, attestation.AttesterSlot, 1.0/float64(attestation.InclusionSlot-attestation.AttesterSlot)*100)
...
func (bigtable *Bigtable) GetValidatorBalanceStatistics(validators []uint64, startEpoch, endEpoch uint64) (map[uint64]*types.ValidatorBalanceStatistic, error) {
	...
	for i := 0; i < len(validators); i += batchSize {
		...
		// log.LogInfo("retrieving validator balance stats for validators %v - %v", vals[0], vals[len(vals)-1])
...
func (bigtable *Bigtable) SaveValidatorIncomeDetails(epoch uint64, rewards map[uint64]*itypes.ValidatorEpochIncome) error {
	...
	log.LogInfo("exported validator income details for epoch %v to bigtable in %v", epoch, time.Since(start))
	return nil
}
		...
		g.Go(func() error {
			...
			err := bigtable.tableValidatorsHistory.ReadRows(ctx, ranges, func(r gcp_bigtable.Row) bool {
				...
				for _, ri := range r[INCOME_DETAILS_COLUMN_FAMILY] {
					...
					if err != nil {
						log.LogError(err, "error decoding validator income data for row", 0, map[string]interface{}{"row": r.Key})
						return false
					}
...
func (bigtable *Bigtable) GetAggregatedValidatorIncomeDetailsHistory(validators []uint64, startEpoch uint64, endEpoch uint64) (map[uint64]*itypes.ValidatorEpochIncome, error) {
	...
	for i := 0; i < len(validators); i += batchSize {
		...
		log.LogInfo("retrieving validator income stats for validators %v - %v", vals[0], vals[len(vals)-1])
...
func GetCurrentDayClIncome(validator_indices []uint64) (map[uint64]int64, error) {
	...
	// aggregate all epoch income data to total day income for each validator
...
func (bigtable *Bigtable) MigrateIncomeDataV1V2Schema(epoch uint64) error {
	...
	err := bigtable.tableBeaconchain.ReadRows(ctx, prefixEpochRange, func(r gcp_bigtable.Row) bool {
		// log.LogInfo("processing row %v", r.Key())
		...
		if epoch != rowKeyEpoch {
			log.LogError(fmt.Errorf("retrieved different epoch than requested, requested: %d, retrieved: %d", epoch, rowKeyEpoch), "", 0)
		}

		// log.LogInfo("epoch is %d", rowKeyEpoch)
		...
		for columnFamily, readItems := range r {
			for _, ri := range readItems {
				...
				// log.LogInfo("retrieved field %s from column family %s for validator %d", ri.Column, columnFamily, validator)
				if columnFamily == INCOME_DETAILS_COLUMN_FAMILY {
					...
					// log.LogInfo("processing income details data for validator %d", validator)
					incomeDetails := &itypes.ValidatorEpochIncome{}
					err = proto.Unmarshal(ri.Value, incomeDetails)
					if err != nil {
						log.LogError(err, "error decoding validator income data for row", 0, map[string]interface{}{"row": r.Key})
					...
					log.LogError(fmt.Errorf("retrieved unexpected column family %s", columnFamily), "", 0)
				}
			}
		}
		return true

...
// BridgeStore contains all the queued requests made on execution layer that impact the consensus layer
...
func (store BridgeSQLStore) SetConsolidationRequests(request ConsolidationRequest) error {
	...
	`, request.TxHash, request.TxIndex, request.ItxIndex, request.BlockNumber, request.BlockTimestamp, request.From, request.Fee, request.SourceAddress, request.SourcePubKey, request.TargetPubKey)
	if err != nil {
		return err
	}
	return nil
...
func (store BridgeSQLStore) SetWithdrawalRequest(request WithdrawalRequest) error {
	...
	`, request.TxHash, request.TxIndex, request.ItxIndex, request.BlockNumber, request.BlockTimestamp, request.From, request.Fee, request.SourceAddress, request.ValidatorPubKey, request.Amount)
	if err != nil {
		return err
	}
	return nil

...
type SwitchToCompoundingEventTransformer struct{}

func (d *SwitchToCompoundingEventTransformer) Transform(tx *sqlx.Tx, events []types.ConsensusLayerEvent) (int, error) {
	...
	newRows := make([]types.SwitchToCompoundingRequestDBRow, len(data))
	for i, event := range data {
		slotQueued := int64(event.Slot)
		indexQueued := int64(event.EventIndex)
		newRows[i] = types.SwitchToCompoundingRequestDBRow{
	...
	for chunk := range slices.Chunk(newRows, 2000) {
		_, err = tx.NamedExec(`
			INSERT INTO 
			blocks_switch_to_compounding_requests_v2 (
					slot_processed,
					index_processed,

...
type RemovedExcessBalanceEventTransformer struct{}

func (d *RemovedExcessBalanceEventTransformer) Transform(tx *sqlx.Tx, events []types.ConsensusLayerEvent) (int, error) {
	...
	newRows := make([]types.RemovedExcessBalanceDBRow, len(data))
	for i, event := range data {
		slotQueued := int64(event.Slot)
		indexQueued := int64(event.EventIndex)
		newRows[i] = types.RemovedExcessBalanceDBRow{
	...
	for chunk := range slices.Chunk(newRows, 2000) {
		_, err = tx.NamedExec(`
			INSERT INTO 
			blocks_removed_excess_balance_events (
					slot_processed,
					index_processed,

...
type DepositRejectedPostQueueEventTransformer struct{}

func (d *DepositRejectedPostQueueEventTransformer) Transform(tx *sqlx.Tx, events []types.ConsensusLayerEvent) (int, error) {
	eventName := types.DepositRejectedEventName
	...
	data := make([]types.DepositRejectedEvent, len(events))
	...
	var filteredData []types.DepositRejectedEvent
...
outer:
	for _, event := range data {
		...
		log.Warnf("no prestate found for event with pubkey %x, withdrawal_credentials %x, signature %x, amount %d",
			event.Pubkey,
	...
	for chunk := range slices.Chunk(updatedRows, 2000) {
		...
		_, err = tx.Exec(`
			...
			WHERE blocks_deposit_requests_v2.id = temp.id
		`)
		if err != nil {

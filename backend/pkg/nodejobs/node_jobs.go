...
func GetNodeJob(id string) (*types.NodeJob, error) {
	...
	job := types.NodeJob{}
	err := db.WriterDb.Get(&job, `select id, type, status, created_time, submitted_to_node_time, completed_time, data from node_jobs where id = $1`, id)
	if err != nil {
		return nil, err
	}
	err = job.ParseData()
...
func GetNodeJobValidatorInfos(job *types.NodeJob) ([]types.NodeJobValidatorInfo, error) {
	...
	err := db.WriterDb.Select(&dbValis, `select validatorindex, pubkey, withdrawalcredentials, exitepoch, status from validators where validatorindex = any($1)`, pq.Array(indicesArr))
	if err != nil {
		return nil, err
	}
	jobStatus := "Pending"
...
func CreateBLSToExecutionChangesNodeJob(nj *types.NodeJob) (*types.NodeJob, error) {
	...
	err := db.WriterDb.Select(&dbValis, `select validatorindex, pubkey, withdrawalcredentials from validators where validatorindex = any($1)`, pq.Array(indicesArr))
	if err != nil {
		return nil, err
	}
	...
	defer func() {
		err = tx.Rollback()
		if err != nil {
			log.Error(err, "error rolling back db-tx", 0)
		}
	}()
	...
	err = tx.Commit()
	if err != nil {
		return nil, fmt.Errorf("error committing db-tx: %w", err)
	}

	log.InfoWithFields(log.Fields{"id": nj.ID, "type": nj.Type, "validators": len(indicesArr)}, "created node_job")
	return nj, nil
}

func UpdateBLSToExecutionChangesNodeJobs() error {
	jobs := []*types.NodeJob{}
	err := db.WriterDb.Select(&jobs, `select id, type, status, created_time, submitted_to_node_time, completed_time, data from node_jobs where type = $1 and status = $2`, types.BLSToExecutionChangesNodeJobType, types.SubmittedToNodeNodeJobStatus)
	if err != nil {
		return err
	}
...
func UpdateBLSToExecutionChangesNodeJob(job *types.NodeJob) error {
	...
	log.InfoWithFields(log.Fields{"id": job.ID, "type": job.Type, "status": job.Status, "validators": len(indicesArr)}, "checking node_job")
	err := db.WriterDb.Select(&dbValis, `select validatorindex, withdrawalcredentials from validators where validatorindex = any($1)`, pq.Array(indicesArr))
	if err != nil {
		return err
	}
	...
	for _, v := range dbValis {
		...
		if len(toCheck) == 0 {
			...
			_, err = db.WriterDb.Exec(`update node_jobs set status = $1, completed_time = $2 where id = $3`, types.CompletedNodeJobStatus, job.CompletedTime.Time, job.ID)
			if err != nil {
				return err
			}
			log.InfoWithFields(log.Fields{"id": job.ID, "type": job.Type, "status": types.CompletedNodeJobStatus, "validators": len(indicesArr)}, "updated node_job")
		}
	}
	return nil
}
...
func SubmitBLSToExecutionChangesNodeJobs() error {
	...
	jobs := []*types.NodeJob{}
	err := db.WriterDb.Select(&jobs, `select id, type, status, created_time, submitted_to_node_time, completed_time, data from node_jobs where type = $1 and status = $2 order by created_time limit $4-(select count(*) from node_jobs where type = $1 and status = $3)`, types.BLSToExecutionChangesNodeJobType, types.PendingNodeJobStatus, types.SubmittedToNodeNodeJobStatus, maxSubmittedJobs)
	if err != nil {
		return err
	}
	for _, job := range jobs {
...
func SubmitBLSToExecutionChangesNodeJob(job *types.NodeJob) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.WarnWithFields(log.Fields{"data": string(d), "status": resp.Status, "jobID": job.ID}, "failed submitting a job")
	}
	job.Status = jobStatus
	job.SubmittedToNodeTime.Time = time.Now()
	job.SubmittedToNodeTime.Valid = true
	_, err = db.WriterDb.Exec(`update node_jobs set status = $1, submitted_to_node_time = $2 where id = $3`, job.Status, job.SubmittedToNodeTime.Time, job.ID)
	if err != nil {
		return err
	}
	log.InfoWithFields(log.Fields{"id": job.ID, "type": job.Type, "status": jobStatus}, "submitted node_job")
	return nil
}
...
func CreateVoluntaryExitNodeJob(nj *types.NodeJob) (*types.NodeJob, error) {
	...
	err := db.WriterDb.Get(&vali, `select pubkey, status from validators where validatorindex = $1`, njd.Message.ValidatorIndex)
	if err != nil {
		return nil, err
	}
	...
	_, err = db.WriterDb.Exec(`insert into node_jobs (id, type, status, data, created_time) values ($1, $2, $3, $4, now())`, nj.ID, nj.Type, nj.Status, nj.RawData)
	if err != nil {
		return nil, err
	}
	log.InfoWithFields(log.Fields{"id": nj.ID, "type": nj.Type}, "created node_job")
	return nj, nil
}

func UpdateVoluntaryExitNodeJobs() error {
	jobs := []*types.NodeJob{}
	err := db.WriterDb.Select(&jobs, `select id, type, status, created_time, submitted_to_node_time, completed_time, data from node_jobs where type = $1 and status = $2`, types.VoluntaryExitsNodeJobType, types.SubmittedToNodeNodeJobStatus)
	if err != nil {
		return err
	}
	for _, job := range jobs {
...
func UpdateVoluntaryExitNodeJob(job *types.NodeJob) error {
	...
	err := db.WriterDb.Get(&dbResult, `select status from validators where validatorindex = $1`, uint64(jobData.Message.ValidatorIndex))
	...
	if strings.HasPrefix(dbResult.Status, "exit") {
		...
		job.CompletedTime.Time = time.Now()
		job.CompletedTime.Valid = true
		_, err = db.WriterDb.Exec(`update node_jobs set status = $1, completed_time = $2 where id = $3`, job.Status, job.CompletedTime.Time, job.ID)
		if err != nil {
			return err
		}
	}
...
func SubmitVoluntaryExitNodeJobs() error {
	...
	jobs := []*types.NodeJob{}
	err := db.WriterDb.Select(&jobs, `select id, type, status, created_time, submitted_to_node_time, completed_time, data from node_jobs where type = $1 and status = $2 order by created_time limit $4-(select count(*) from node_jobs where type = $1 and status = $3)`, types.VoluntaryExitsNodeJobType, types.PendingNodeJobStatus, types.SubmittedToNodeNodeJobStatus, maxSubmittedJobs)
	if err != nil {
		return err
	}
	for _, job := range jobs {
...
func SubmitVoluntaryExitNodeJob(job *types.NodeJob) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		jobStatus = types.FailedNodeJobStatus
		log.WarnWithFields(log.Fields{"res": string(d), "status": resp.Status, "jobID": job.ID, "jobType": job.Type}, "failed submitting a job")
	}
	job.Status = jobStatus
	job.SubmittedToNodeTime.Time = time.Now()
	job.SubmittedToNodeTime.Valid = true

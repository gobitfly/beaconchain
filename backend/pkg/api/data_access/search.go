...
func (d *DataAccessService) GetSearchValidatorsByDepositAddress(ctx context.Context, chainId uint64, address []byte) (*t.SearchValidatorsByDepositAddress, error) {
	...
	err := db.ReaderDb.Select(&ret.Count, "select count(validatorindex) from validators where pubkey in (select publickey from eth1_deposits where from_address = $1);", address)
	if err != nil {
		return nil, err
	}
	if ret.Count == 0 {
...
func (d *DataAccessService) GetSearchValidatorsByWithdrawalCredential(ctx context.Context, chainId uint64, credential []byte) (*t.SearchValidatorsByWithdrwalCredential, error) {
	...
	err := db.ReaderDb.Select(&ret.Count, "select count(validatorindex) from validators where withdrawalcredentials = $1;", credential)
	if err != nil {
		return nil, err
	}
	if ret.Count == 0 {
...
func (d *DataAccessService) GetSearchValidatorsByGraffiti(ctx context.Context, chainId uint64, graffiti string) (*t.SearchValidatorsByGraffiti, error) {
	...
	err := db.ReaderDb.Select(&ret.Count, "select count(distinct proposer) from blocks where graffiti_text = $1;", graffiti) // added a limit here to keep the query fast
	if err != nil {
		return nil, err
	}
	if ret.Count == 0 {

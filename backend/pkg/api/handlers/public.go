...
func (h *HandlerService) PublicGetValidatorDashboardHeatmap(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	data, err := h.getDataAccessor(ctx).GetValidatorDashboardHeatmap(ctx, *dashboardId, protocolModes, aggregation, afterTs, beforeTs)
	if err != nil {
		handleErr(w, r, err)
		return
	}
...
func (h *HandlerService) PublicGetValidatorDashboardGroupHeatmap(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	data, err := h.getDataAccessor(ctx).GetValidatorDashboardGroupHeatmap(ctx, *dashboardId, groupId, protocolModes, aggregation, requestedTimestamp)
	if err != nil {
		handleErr(w, r, err)
		return
	}

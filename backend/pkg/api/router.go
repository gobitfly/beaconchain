...
func addValidatorDashboardRoutes(hs *handlers.HandlerService, publicRouter, internalRouter *mux.Router, cfg *types.Config) {
	...
	endpoints := []endpoint{
		...
		{http.MethodGet, "/{dashboard_id}/consolidations", handlers.Handle(http.StatusOK, hs.GetValidatorDashboardConsolidations, allowMocking), handlers.Handle(http.StatusOK, hs.GetValidatorDashboardConsolidations, allowMocking)},

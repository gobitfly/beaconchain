...
func addValidatorDashboardRoutes(hs *handlers.HandlerService, publicRouter, internalRouter *mux.Router, cfg *types.Config) {
	...
	endpoints := []endpoint{
		...
		{http.MethodGet, "/{dashboard_id}/rocket-pool/minipools/{node_address}", hs.PublicGetValidatorDashboardRocketPoolMinipools, hs.InternalGetValidatorDashboardRocketPoolMinipools},
	}

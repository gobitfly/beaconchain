...
const (
	...
	routeGetRowsRange    = "/rowsRange"
	routeGetRow          = "/row"
	routeRead            = "/read"
	routeGetRowsWithKeys = "/rowsWithKeys"
)

// RemoteServer is a dev utility that can be run on a server with sufficient IAM credential
// it opens a http API on top of Database interface
// making it accessible through an ssh tunel from a local dev environment without any IAM credential
// DO NOT USE IN PROD

...
func TestStore(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			limit:          2,
			opts:           []Option{OnlyTransfers(), ByAsset(usdc), WithTimeRange(timestamppb.New(t0.Add(1*time.Second)), timestamppb.New(t0.Add(3*time.Second)))},
			addresses:      []common.Address{alice},
		...
		},
		{
			...
			limit:          2,
			opts:           []Option{OnlyTransfers(), OnlySent(), ByAsset(usdc), WithTimeRange(timestamppb.New(t0.Add(1*time.Second)), timestamppb.New(t0.Add(3*time.Second)))},
			addresses:      []common.Address{alice},
		...
		},
		{
			...
			limit:          2,
			opts:           []Option{OnlyTransfers(), OnlyReceived(), ByAsset(usdc), WithTimeRange(timestamppb.New(t0.Add(1*time.Second)), timestamppb.New(t0.Add(3*time.Second)))},
			addresses:      []common.Address{alice},
			expectedHashes: []string{"hash3"},
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for chainID, blocks := range tt.txs {
				for _, txs := range blocks {
					if _, err := store.AddIndexedBlock(IndexedBlock{
						ChainID:      chainID,
						Transactions: txs,
					}); err != nil {
						t.Fatal(err)

...
func NewLighthouseClient(cl *consapi.NodeImplRetriever, chainID *big.Int) (*LighthouseClient, error) {
...
func (lc *LighthouseClient) GetNewBlockChan() chan *types.Block {
	...
	go func() {
		...
		if err != nil {
			log.LogFatal(err, "getting eventsource stream error", 0)
		}
		defer stream.Close()
		...
		for {
			...
			case err := <-stream.Errors:
				log.LogError(err, "Lighthouse connection error (will automatically retry to connect)", 0)
			case e := <-stream.Events:
				...
				if err != nil {
					log.LogWarn("failed to decode block event: %v", err)
					continue
				}

				log.LogInfo("retrieving data for slot %v", parsed.Slot)
				block, err := lc.GetBlockBySlot(uint64(parsed.Slot))
				if err != nil {
					log.LogWarn("failed to fetch block for slot %d: %v", uint64(parsed.Slot), err)
					continue
				}
				log.LogInfo("retrieved block for slot %v", parsed.Slot)
...
func (lc *LighthouseClient) GetValidatorQueue() (*types.ValidatorQueue, error) {
	...
	validatorsResp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/head/validators?status=pending_queued,active_exiting,active_slashed", lc.cl.Endpoint))
	if err != nil {
...
func (lc *LighthouseClient) GetEpochAssignments(epoch uint64) (*types.EpochAssignments, error) {
	...
	committeesResp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/%s/committees?epoch=%d", lc.cl.Endpoint, depStateRoot, epoch))
	if err != nil {
...
func (lc *LighthouseClient) GetEpochProposerAssignments(epoch uint64) (*StandardProposerDutiesResponse, error) {
	...
	proposerResp, err := lc.get(fmt.Sprintf("%s/eth/v1/validator/duties/proposer/%d", lc.cl.Endpoint, epoch))
	if err != nil {
		return nil, fmt.Errorf("error retrieving proposer duties for epoch %v: %w", epoch, err)
	}
...
func (lc *LighthouseClient) GetValidatorState(epoch uint64) (*StandardValidatorsResponse, error) {
	validatorsResp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/%d/validators", lc.cl.Endpoint, epoch*utils.Config.Chain.ClConfig.SlotsPerEpoch))
	if err != nil && epoch == 0 {
		validatorsResp, err = lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/%v/validators", lc.cl.Endpoint, "genesis"))
		if err != nil {
...
func (lc *LighthouseClient) GetEpochData(epoch uint64, skipHistoricBalances bool) (*types.EpochData, error) {
	...
	log.LogInfo("retrieved data for %v validators for epoch %v", len(data.Validators), epoch)
	...
	wg.Go(func() error {
		...
		log.LogInfo("retrieved validator assignment data for epoch %v", epoch)
		return nil
	})
		...
		wg.Go(func() error {
			...
			if err != nil {
				if strings.HasSuffix(err.Error(), "can't be retrieved as it hasn't finished yet") { // should no longer happen
					log.LogWarn("error retrieving epoch participation statistics for epoch %v: %v", epoch, err)
				} else {
	...
	wg.Go(func() error {
		for slot := epoch * utils.Config.Chain.ClConfig.SlotsPerEpoch; slot <= (epoch+1)*utils.Config.Chain.ClConfig.SlotsPerEpoch-1; slot++ {
			...
			mux.Unlock()
			log.LogInfo("processed data for current epoch slot %v in %v", slot, time.Since(start))
		}
		return nil
	})
	...
	wg.Go(func() error {
		for slot := (epoch + 1) * utils.Config.Chain.ClConfig.SlotsPerEpoch; slot <= (epoch+2)*utils.Config.Chain.ClConfig.SlotsPerEpoch-1; slot++ {
			...
			mux.Unlock()
			log.LogInfo("processed data for next epoch slot %v in %v", slot, time.Since(start))
		}
		return nil
	})
	...
	log.LogInfo("retrieved %v blocks for epoch %v", len(data.Blocks), epoch)
...
func (lc *LighthouseClient) GetBalancesForEpoch(epoch int64) (map[uint64]uint64, error) {
	...
	resp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/%d/validator_balances", lc.cl.Endpoint, epoch*int64(utils.Config.Chain.ClConfig.SlotsPerEpoch)))
	if err != nil && epoch == 0 {
		resp, err = lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/genesis/validator_balances", lc.cl.Endpoint))
		if err != nil {
...
func (lc *LighthouseClient) GetBlockByBlockroot(blockroot []byte) (*types.Block, error) {
	resHeaders, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/headers/0x%x", lc.cl.Endpoint, blockroot))
	if err != nil {
		if err == errNotFound {
			// no block found
	...
	resp, err := lc.get(fmt.Sprintf("%s/eth/v2/beacon/blocks/%s", lc.cl.Endpoint, parsedHeaders.Data.Root))
	if err != nil {
	...
	err = json.Unmarshal(resp, &parsedResponse)
	if err != nil {
		log.LogError(err, "error parsing block data for slot", 0, map[string]interface{}{"slot": parsedHeaders.Data.Header.Message.Slot})
...
func (lc *LighthouseClient) GetBlockHeader(slot uint64) (*StandardBeaconHeaderResponse, error) {
	...
	resHeaders, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/headers/%d", lc.cl.Endpoint, slot))
	if err != nil && slot == 0 {
		headResp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/headers", lc.cl.Endpoint))
		if err != nil {
...
func (lc *LighthouseClient) GetBlockBySlot(slot uint64) (*types.Block, error) {
	...
	if ok {
		...
		if ok {
			log.LogInfo("retrieved slot %v (0x%x) from in memory cache", block.Slot, block.BlockRoot)
			return block, nil
		} else {
			log.LogError(fmt.Errorf("unable to convert cached block to block type"), "", 0)
		}
	}
	lc.slotsCacheMux.Unlock()
		...
		for i := uint64(0); i < aggregationBits.Len(); i++ {
			if aggregationBits.BitAt(i) {
				...
				if !found { // This should never happen!
					validator = 0
					log.LogFatal(fmt.Errorf("error retrieving assigned validator for attestation %v of block %v for slot %v committee index %v member index %v", i, block.Slot, a.Data.Slot, a.Data.CommitteeIndex, i), "", 0)
				}
...
func (lc *LighthouseClient) GetValidatorParticipation(epoch uint64) (*types.ValidatorParticipation, error) {
	...
	log.LogInfo("requesting validator inclusion data for epoch %v", request_epoch)
...
func (lc *LighthouseClient) GetSyncCommittee(stateID string, epoch uint64) (*StandardSyncCommittee, error) {
	syncCommitteesResp, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/states/%s/sync_committees?epoch=%d", lc.cl.Endpoint, stateID, epoch))
	if err != nil {
...
func (lc *LighthouseClient) GetBlobSidecars(stateID string) (*StandardBlobSidecarsResponse, error) {
	res, err := lc.get(fmt.Sprintf("%s/eth/v1/beacon/blob_sidecars/%s", lc.cl.Endpoint, stateID))
	if err != nil {

...
func (bigtable *Bigtable) TransformBlock(block *types.Eth1Block, cache *freecache.Cache) (bulkData *types.BulkMutations, bulkMetadataUpdates *types.BulkMutations, err error) {
	...
	for _, t := range block.GetTransactions() {
		price := big.NewInt(int64(t.GasPrice))
		...
		txFee := new(big.Int).Mul(big.NewInt(int64(t.GasPrice)), big.NewInt(int64(t.GasUsed)))

		if len(block.BaseFee) > 0 {
			effectiveGasPrice := math.BigMin(new(big.Int).Add(big.NewInt(int64(t.MaxPriorityFeePerGas)), new(big.Int).SetBytes(block.BaseFee)), big.NewInt(int64(t.MaxFeePerGas)))
		...
		for _, itx := range t.Itx {
			if itx.Path == "[]" || itx.Value == "0" { // skip top level call & empty calls
				continue
			}
	...
	for _, tx := range block.GetTransactions() {
		for _, itx := range tx.GetItx() {
			if common.HexToAddress(itx.To) == common.BytesToAddress(block.GetCoinbase()) {
				...
				mevReward = new(big.Int).Add(mevReward, big.NewInt(int64(value)))
			}
		}
	}
	return mevReward
...
func CalculateTxFeeFromTransaction(tx *types.Eth1Transaction, blockBaseFee *big.Int) *big.Int {
	...
	switch tx.Type {
	case 0, 1:
		txFee.Mul(txFee, big.NewInt(int64(tx.GasPrice)))
	case 2, 3:
		// multiply gasused with min(baseFee + maxpriorityfee, maxfee)
		if normalGasPrice, maxGasPrice := new(big.Int).Add(blockBaseFee, big.NewInt(int64(tx.MaxPriorityFeePerGas))), big.NewInt(int64(tx.MaxFeePerGas)); normalGasPrice.Cmp(maxGasPrice) <= 0 {

...
func SaveRocketPoolConfig(storageAddress, poolAddress []byte) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func SaveRocketPoolMiniPools(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func UpdateRocketPoolMiniPools() error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func SaveRocketPoolNodes(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func SaveRocketPoolRewardTree(id uint64, data []byte) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec(`INSERT INTO rocketpool_reward_tree (id, data) 
			VALUES($1, $2) 
			ON CONFLICT DO NOTHING`, id, data)
...
func RefreshRocketPoolMV() error {
	_, err := WriterDb.Exec(`REFRESH MATERIALIZED VIEW CONCURRENTLY rocketpool_rewards_summary`)
	return err
}

func SaveRocketPoolDAOProposals(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec(
		fmt.Sprintf(`INSERT INTO rocketpool_dao_proposals (
				rocketpool_storage_address, 
				id, 
				dao, 
				proposer_address,
...
func SaveRocketPoolDAOProposalVotes(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func SaveRocketPoolDAOMembers(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func DeleteRocketPoolDAOMembers(addresses [][]byte) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}
...
func SaveRocketPoolNetworkStats(
	rlpPrice, claimIntervalTime string,
	claimIntervalTimeStart time.Time, currentNodeFee float64,
	currentNodeDemand, rEthSupply, nodeOperatorRewards string,
	rEthExchangeRate float64,
	nodeCount, minipoolCount, oDAOMemberCount int,
	totalEthStaking, totalEthBalance string,
) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	...
	_, err = tx.Exec(`
		...
		)`, rlpPrice, claimIntervalTime, claimIntervalTimeStart,
		currentNodeFee, currentNodeDemand, rEthSupply, nodeOperatorRewards,
		rEthExchangeRate, nodeCount, minipoolCount, oDAOMemberCount,
		totalEthStaking, totalEthBalance)
...
func SaveValidatorPool(valueStrings []string, valueArgs []interface{}) error {
	tx, err := WriterDb.Beginx()
	if err != nil {
		return err
	}

...
func (t *Transformer) Tx(chainID string, blk *types.Eth1Block) (map[string][]database.Item, map[string][]database.Item, error) {
	...
	for _, tx := range blk.Transactions {
		...
		fee := new(big.Int).Mul(new(big.Int).SetUint64(tx.GetGasPrice()), big.NewInt(int64(tx.GetGasUsed()))).Bytes()
		blobFee := new(big.Int).Mul(new(big.Int).SetUint64(tx.GetBlobGasPrice()), big.NewInt(int64(tx.GetBlobGasUsed()))).Bytes()
		indexedTx := &types.Eth1TransactionIndexed{
			...
			From:               tx.GetFrom(),
			To:                 to,
			Value:              big.NewInt(int64(tx.GetValue())).Bytes(),
			TxFee:              fee,
			GasPrice:           big.NewInt(int64(tx.GetGasPrice())).Bytes(),
			IsContractCreation: isContract,
			ErrorMsg:           "",
			BlobTxFee:          blobFee,
			BlobGasPrice:       big.NewInt(int64(tx.GetBlobGasPrice())).Bytes(),

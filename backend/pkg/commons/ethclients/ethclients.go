...
type EthClients struct {
	...
	ClientReleaseDate    int64 // 0 = searching
	NetworkShare         string
...
func fetchClientData(repo string) *gitAPIResponse {
	...
	if err != nil {
		log.Error(err, "error retrieving ETH Client Data", 0)
		return nil
	}
	...
	if resp.StatusCode != http.StatusOK {
		log.Error(nil, "error retrieving ETH Client Data", 0, log.Fields{"status code": resp.StatusCode})
		return nil
	}
	...
	if err != nil {
		log.Error(err, "error decoding ETH Clients json response to struct", 0)
		return nil
	}
...
func fetchClientNetworkShare() []ethernodesAPIStruct {
	...
	if err != nil {
		log.Error(err, "error retrieving ETH Clients Network Share Data", 0)
	...
	if err != nil {
		log.Error(err, "error decoding ETH Clients Network Share json response to struct", 0)
	}
...
func prepareEthClientData(repo string, name string, curTime time.Time) (string, int64) {
	...
	if len(date) == 2 {
		...
		if err != nil {
			log.Error(err, "error parsing git repo.", 0)
			return client.Name, 0 // client.Name is client version from github api
		}
	...
	return "Github", 0 // If API limit is exceeded
}
...
func updateEthClient() {
	...
	log.Infof("Updating ETH Clients Information")
	ethClientsMux.Lock()

...
func updatePrices() {
	...
	if err = addPricePairs(mainCurrency); err != nil {
		log.Error(err, "error calculating price pairs", 0)
		return
	}
...
// calculates all prices in a given currency
func addPricePairs(currency string) error {
	pricesMu.Lock()
	defer pricesMu.Unlock()
	pricesCopy := prices

...
type ConsolidationProcessedEventTransformer struct{}

func (d *ConsolidationProcessedEventTransformer) Transform(tx *sqlx.Tx, events []types.ConsensusLayerEvent) (int, error) {
	...
	for _, event := range data {
		for i, preStateRow := range preState {
			if bytes.Equal(preStateRow.SourcePubkey, event.SourcePubkey) &&
				...
				preStateRow.AmountConsolidated = &event.Amount

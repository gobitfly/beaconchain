...
type ConsolidationRejectedPostQueueEventTransformer struct{}

func (d *ConsolidationRejectedPostQueueEventTransformer) Transform(tx *sqlx.Tx, events []types.ConsensusLayerEvent) (int, error) {
	eventName := types.ConsolidationRejectedEventName
	...
	data := make([]types.ConsolidationRejectedEvent, len(events))
	...
	var filteredData []types.ConsolidationRejectedEvent
	...
	for _, event := range data {
		orConditions = append(orConditions, goqu.Ex{
			"source_pubkey": event.SourcePubkey,
			"target_pubkey": event.TargetPubkey,
		})
	}
	ds := goqu.Dialect("postgres").From("blocks_consolidation_requests_v2").Select(goqu.Star()).Where(
		goqu.And(
	...
	// execute query
	var preState []types.ConsolidationRequestDBRow
	err = tx.Select(&preState, query, args...)
	if err != nil {
		return 0, errors.Wrap(err, "error executing sql query")
	}
	var updatedRows []types.ConsolidationRequestDBRow
	// update in-memory state with events
outer:
	for _, event := range data {
		for i, preStateRow := range preState {
			if bytes.Equal(preStateRow.SourcePubkey, event.SourcePubkey) &&
				bytes.Equal(preStateRow.TargetPubkey, event.TargetPubkey) {
		...
		log.Warnf("no prestate found for event with source_pubkey %x, target_pubkey %x",
			event.SourcePubkey,
			event.TargetPubkey,
		)
	...
	for chunk := range slices.Chunk(updatedRows, 2000) {
		...
		_, err = tx.Exec(`
			UPDATE blocks_consolidation_requests_v2
			SET
			...
			WHERE blocks_consolidation_requests_v2.id = temp.id
		`)
		if err != nil {

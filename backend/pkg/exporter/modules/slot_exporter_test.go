...
func TestSlotExporter(t *testing.T) {
	...
	t.Run("genesis slot", func(t *testing.T) {
		...
		// mock DB calls
		mockDB.On("BeginTx").Return(mockTx, nil).Once()
		mockDB.On("RollbackTx", mockTx).Return(nil).Maybe()
		mockDB.On("CommitTx", mockTx).Return(nil).Once()
		...
		mockBT.On("SaveValidatorBalances",
			utils.EpochOfSlot(mockBlock.Slot),
			mockBlock.Validators,
		).Return(nil)

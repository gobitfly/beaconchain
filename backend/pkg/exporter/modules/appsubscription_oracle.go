...
func checkSubscriptions() {
	...
	for {
		...
		if err != nil {
			log.LogError(err, "error retrieving subscription data from db: %v", 0, nil)
			return
		}

		googleClient, err := initGoogle()
		if googleClient == nil {
			log.LogError(err, "error initializing google client: %v", 0, nil)
			return
		}

		appleClient, err := initApple()
		if err != nil {
			log.LogError(err, "error initializing apple client: %v", 0, nil)
			return
		}
		...
		for _, receipt := range receipts {
			...
			if err != nil {
				...
				if strings.Contains(err.Error(), "expired") {
					...
					if err != nil {
						log.LogError(err, "subscription set expired failed", 0, map[string]interface{}{"receiptID": receipt.ID})
					}
					continue
				}
				log.LogWarn("subscription verification failed in service for [%v]: %v", receipt.ID, err)
				continue
			}
		...
		log.LogInfoWithFields(log.Fields{"subscriptions": len(receipts), "duration": time.Since(start)}, "subscription update completed")
...
func verifyApple(apple *api.StoreClient, receipt *types.PremiumData) (*VerifyResponse, error) {
	...
	if len(receipt.Receipt) > 100 {
		...
		if err != nil {
			log.LogError(err, "error resolving legacy appstore receipt", 0, nil)
...
func updateValidationState(receipt *types.PremiumData, validation *VerifyResponse) {
	...
	if err != nil {
		log.LogError(err, "error updating subscription state", 0)
	}
	...
	if err != nil {
		log.LogError(err, "error updating subscription product id", 0)
	}
}

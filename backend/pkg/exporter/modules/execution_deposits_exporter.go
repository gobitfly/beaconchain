...
func (d *executionDepositsExporter) exportTillBlock(block uint64) (err error) {
	...
	for blockOffset < blockTarget {
		...
		for blockOffset < blockTarget && len(depositsToSave) <= depositsToSaveBatchSize && blockOffset < blockOffset+blockBatchSize {
		...
		if len(depositsToSave) > 0 {
			log.Infof("exported deposits for blocks %d-%d: %d", blockBatchStart, d.LastExportedBlock, len(depositsToSave))
		}

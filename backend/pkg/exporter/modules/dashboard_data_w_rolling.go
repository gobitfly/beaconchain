	...
	tmpl := `
		WITH
			...
			ON CONFLICT {{ .TableConflict }} DO UPDATE SET
					...
					blocks_cl_attestations_reward = COALESCE({{ .TableTo }}.blocks_cl_attestations_reward, 0) + EXCLUDED.blocks_cl_attestations_reward,
					blocks_cl_sync_aggregate_reward = COALESCE({{ .TableTo }}.blocks_cl_sync_aggregate_reward, 0) + EXCLUDED.blocks_cl_sync_aggregate_reward,
					...
					blocks_expected = COALESCE({{ .TableTo }}.blocks_expected, 0) + EXCLUDED.blocks_expected,
					sync_committees_expected = COALESCE({{ .TableTo }}.sync_committees_expected, 0) + EXCLUDED.sync_committees_expected,

...
func AddToRollingCustom(tx *sqlx.Tx, custom CustomRolling) error {
	...
	// 	WITH
	// 		head_balance_ends as (
	// 			SELECT validator_index, balance_end FROM %[6]s WHERE %[5]s = $2 -- from table, from epoch column
	// 		),
	// 		%[2]s -- balance start query
	// 		aggregate_head as (
	// 			SELECT
	// 				validator_index,
	// 				SUM(attestations_source_reward) as attestations_source_reward,
	...
	// 				MAX(last_executed_duty_epoch) as last_executed_duty_epoch
	// 			FROM %[6]s
	// 			WHERE %[5]s >= $1 AND %[5]s <= $2 -- epoch column
	// 			GROUP BY validator_index
	// 		)
	// 		INSERT INTO %[1]s (
	// 			%[7]s
	...
	// 				last_executed_duty_epoch =  COALESCE(EXCLUDED.last_executed_duty_epoch, %[1]s.last_executed_duty_epoch)
	// `,
	// 	custom.TableTo, custom.TailBalancesQuery, custom.TailBalancesJoinQuery, custom.TailBalancesInsertColumnQuery, custom.TableFromEpochColumn, custom.TableFrom, custom.TableDayColum, custom.TableDayValue, custom.TableConflict)

	//fmt.Printf("total query: %s", queryBuffer.String())

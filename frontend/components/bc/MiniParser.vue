 ...
 *    You can also pass an object containing numbers:
 *      <BcMiniParser :input="stringOrArrayOfStrings" :numbers="{easterDuration: 50}" />
 *    (see the example just below to understand the purpose of :links and :numbers)
 *
 *  Example of input:
 *
 *    # About chocolate
 ...
 *      numbers in the object passed in prop :numbers can be inserted by writing $objectKey. They will be formatted according to the local settings of the user.
...
const props = defineProps<{ input: string|string[], links?: Record<string, string>, numbers?: Record<string, number> }>()
...
function parseText (text: string) : VDOMnodes {
  ...
  do {
    ...
    if (openingTagPos < 0 || closingTagPos < 0) { // First case: no tag, we can copy the raw line. Second case: syntax error (either the closing tag has been forgotten or nested tags have their closure swapped)
      output.push(...cleanTextAndInsertVariables(text)) // in both cases we output the text without parsing it
      break
    }
    if (openingTagPos > 0) {
      output.push(...cleanTextAndInsertVariables(text.slice(0, openingTagPos)))
    }
    ...
    switch (openingTag) {
      ...
      case Tag.Code : output.push(h('span', { style: 'font-family: monospace;' }, cleanTextAndInsertVariables(middle, true))); break
...
function parseLink (text: string) : VDOMnodes {
  ...
  if (middleTagPos <= 0) { // syntax error (the link has no caption) : we output the text without parsing it
    return cleanTextAndInsertVariables(text)
  }
...
function cleanTextAndInsertVariables (raw: string, showAsRaw = false) : VDOMnodes {
  const output: VDOMnodes = []
  if (showAsRaw) {
    raw = raw.replaceAll(' ', '\xA0')
  } else {
    for (const key in props.numbers) {
      const sought = Tag.Variable + key
      let pos = -sought.length
      do {
        pos = raw.indexOf(sought, pos + sought.length)
        if (pos >= 1 && raw[pos - 1] === ESC) { continue }
      } while (pos > 0)
    }
  }
  return output.map(part => (typeof part === 'string') ? part.replaceAll(ESC, '') : part)
}

...
const rewardFull = computed(() => converter.value.weiToValue(
  totalElCl(data.value?.rewards.last_30d ?? {
    cl: '0',
    el: '0',
...
const validatorsOffline = computed(() => data.value?.validators.offline ?? 0)
const validatorsOnline = computed(() => data.value?.validators.online ?? 0)
const validatorsInfos = computed(() =>
  [
    {
      label: $t('dashboard.validator.overview.validators_balance.balance_total_tooltip'),
      value: formatValueWei(data.value?.balances.total ?? 0),
    },
    {
      label: $t('dashboard.validator.overview.validators_balance.balance_effective'),
      value: formatValueWei(data.value?.balances.effective ?? 0),
    },
    {
      label: $t('dashboard.validator.overview.validators_balance.balance_staked'),
      value: formatValueWei(data.value?.balances.staked_eth ?? 0),
    },
  ],
)
...
const efficiencyInfos = computed(() =>
  TimeFrames.map(k => ({
    ...
    value: formatPercent(data.value?.efficiency[k] ?? 0),
  })),
)

const rewardsInfos = TimeFrames.map(k =>
  createInfo(k, data.value?.rewards[k] ?? {
    cl: '0',
    el: '0',
  }, formatValueWei),
)

const apr = computed(() => formatPercent(totalElClNumbers(data.value?.apr.last_30d ?? {
  cl: 0,
  el: 0,
})),
)
  ...
  <div class="container">
    ...
    >
      ...
      <template #additionalInfo>
        ...
        <span class="bold">
          {{ formatValueWei(data?.balances.total ?? 0) }}
        </span>
      </template>
    </DashboardValidatorOverviewItem>
    <DashboardValidatorOverviewItem
    ...
    >
      {{ formatToPercent(data?.efficiency.last_24h ?? 0) }}
    </DashboardValidatorOverviewItem>
    <DashboardValidatorOverviewItem
      :infos="rewardsInfos"

...
<template>
  <div class="option-row">
    ...
    >
      ...
      <template #tooltip>
        <div
          v-if="tPath.includes('is_validator_offline_subscribed')"
          class="tt-content"
        >
          {{ tooltipLines[0] }}
          <ul>
        ...
        <div
          v-else-if="tPath.includes('group_offline_threshold')"
          class="tt-content"
        >
          {{ tooltipLines[0] }}
          <ul>
        ...
        <div
          v-else-if="tPath.includes('is_ignore_spam_transactions_enabled')"
          class="tt-content"
        >
          {{ tooltipLines[0] }}
          <b>{{ tooltipLines[1] }}</b>
    ...
    <BcPremiumGem
      v-if="lacksPremiumSubscription"
      class="gem"
    />
    <div
      v-if="parentVmodel.type != 'networks'"
      class="right"
    >
      <div
        v-if="parentVmodel.type == 'amount' || parentVmodel.type == 'percent'"
        class="input"
      >
        <BcInputNumber
      ...
      <span
        v-if="parentVmodel.type == 'percent'"
        :class="deactivationClass"
      >%</span>
      <Checkbox
    ...
    >
      <BcNetworkSelector
        v-if="parentVmodel.networks !== undefined"
        v-model="parentVmodel.networks"
        :class="deactivationClass"
      />
    </div>
  </div>
